...
<div class="w-full grid cols-1 space-y-2 mt-3">
	...
	<div class="grid grid-cols-1 gap-4">
		{#if renderAddress(person, $page.data.instance.country).text.length > 0}
			<div>
				<div class="section-title">Address</div>
				<a
					class="font-medium text-foreground"
					href={renderAddress(person, $page.data.instance.country).url}
					target="_blank">{renderAddress(person, $page.data.instance.country).text}</a
				>
			</div>
		{/if}
