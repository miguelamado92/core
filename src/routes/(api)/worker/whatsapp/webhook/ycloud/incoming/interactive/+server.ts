...
export async function POST(event) {
	try {
		...
		if (parsed.whatsappInboundMessage.type !== 'interactive') {
		...
		if (message.interactive && message.interactive.button_reply.id) {
		...
		return error(
			500,
			'WORKER:/whatsapp/webhooks/ycloud/incoming/interactive/+server.ts',
